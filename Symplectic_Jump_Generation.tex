...
\item One positive validation: a symplectic jump engine over PPMI+SVD embeddings of Python code, augmented with log-bigram corpus statistics, bracket-depth conservation, and a parser-based rejection filter, yielding 78\% parse rate on 18 test prompts with $\sim$2.5\,MB total model size.
...
The natural-language experiments that follow test whether this routing architecture can, combined with the symplectic jump, produce coherent morally-framed output. The experiments falsify this goal for grammatical English. The code experiments of §\ref{sec:code} carry the routing mechanism unchanged to a formal-language substrate where the underlying selection succeeds; the moral routing architecture is orthogonal to the selection-vs-sequencing failure mode identified in §\ref{sec:synthesis}.
...
\textbf{78\% parse rate} across 18 test prompts. The framework works when the prompt is itself a valid Python prefix (\texttt{codeop.compile\_command} returns a code object or \texttt{None}). Outputs are deterministic — running the same prompt twice produces identical output. Outputs are semantically odd in places (e.g., \texttt{"" >8 >10 >12} is a chained-comparison expression coming from f-string format tokens in the training corpus), but they are syntactically valid Python that a parser accepts.
...
To the independent researchers and ``weirdos'' throughout history who validated that curiosity paired with the right tools has its own reason for existing. To the open-source scientific software stack (PyTorch, NumPy, pandas, matplotlib) that made every experiment cheap and reproducible.
